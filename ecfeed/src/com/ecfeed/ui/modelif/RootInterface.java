...
public class RootInterface extends GlobalParametersParentInterface {
	...
	public ClassNode addNewClass(String className){
		...
		if(execute(new RootOperationAddNewClass(getOwnNode(), addedClass, getOwnNode().getClasses().size()), Messages.DIALOG_ADD_NEW_CLASS_PROBLEM_TITLE)){
			return addedClass;
		}
		return null;
	}
		...
		if (dialog.open() == IDialogConstants.OK_ID) {
			...
			if(selectedClass != null){
				...
				try {
					...
					if(execute(new RootOperationAddNewClass(getOwnNode(), classModel, getOwnNode().getClasses().size()), Messages.DIALOG_ADD_NEW_CLASS_PROBLEM_TITLE)){
						return classModel;
					}
