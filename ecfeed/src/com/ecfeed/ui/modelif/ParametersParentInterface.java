...
public abstract class ParametersParentInterface extends AbstractNodeInterface {
	...
	public boolean addParameter(AbstractParameterNode parameter, int index) {
		return execute(new GenericOperationAddParameter(getOwnRootNode(), parameter, index), Messages.DIALOG_CONVERT_METHOD_PROBLEM_TITLE);
	}
	...
	protected String generateNewParameterName() {
		...
		while(getOwnRootNode().getParameter(name) != null){
	...
	@Override
	public ParametersParentNode getOwnRootNode(){
		return (ParametersParentNode)super.getOwnRootNode();
	}
}
